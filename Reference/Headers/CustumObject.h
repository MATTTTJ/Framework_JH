...
BEGIN(Engine)
class ENGINE_DLL CCustumObject final :	public CGameObject
{
public:
...
private:
	CCustumObject(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CCustumObject(const CCustumObject& rhs);
	virtual ~CCustumObject() = default;
...
public:
	static CCustumObject*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const vector<pair<_uint, wstring>>& vecPrototypeInfo, _uint iNumTextureCom);
