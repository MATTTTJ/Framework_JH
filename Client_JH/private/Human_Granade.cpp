...
HRESULT CHuman_Granade::Initialize_Clone(const wstring& wstrPrototypeTag, void* pArg)
{
	...
	m_pHuman_Granade_State = CHuman_Granade_State::Create(this, m_pState, m_pModelCom, m_pTransformCom, m_pNavigationCom);
	FAILED_CHECK_RETURN(Ready_UI(), E_FAIL);

	if (m_tMonsterOption.m_bFirstSpawnType[STATE_ALREADYSPAWN] == true)
	{
		m_pModelCom->Set_CurAnimIndex(CHuman_Granade_State::BOOM_JUSTSTAND);
	}
	else if (m_tMonsterOption.m_bFirstSpawnType[STATE_NODETECTED] == true)
	{
		m_pModelCom->Set_CurAnimIndex(CHuman_Granade_State::BOOM_PATROL);
	}
...
void CHuman_Granade::Tick(_double TimeDelta)
{
	...
	m_pHuman_Granade_State->Tick(TimeDelta);
...
void CHuman_Granade::Late_Tick(_double TimeDelta)
{
	...
	m_pHuman_Granade_State->Late_Tick(TimeDelta);
...
HRESULT CHuman_Granade::Render_ShadowDepth()
{
...
HRESULT CHuman_Granade::Render_OutLineFlag()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
_bool CHuman_Granade::Collision_Detected(CCollider* pOtherCollider)
{
	return m_pColliderCom[COLLTYPE_DETECTED]->Collision(pOtherCollider);
}

_bool CHuman_Granade::Collider_AttRange(CCollider* pOtherCollider)
{
...
void CHuman_Granade::Collision_Body(CBullet* pBullet)
{
	...
	m_pHuman_Granade_State->Set_DamagedState(CHuman_Granade_State::HITBODY);
}

void CHuman_Granade::Collision_Head(CBullet* pBullet)
{
	CBullet::BULLETOPTION BulletDesc;
	BulletDesc = pBullet->Get_BulletOption();
	...
	m_pHuman_Granade_State->Reset_Damaged();
	m_pHuman_Granade_State->Set_DamagedState(CHuman_Granade_State::HIT);
	m_pHuman_Granade_State->Set_DamagedState(CHuman_Granade_State::HITHEAD);
}

void CHuman_Granade::Collision_Hide(CBullet* pBullet)
{
	m_bHideCollision = true;
}

void CHuman_Granade::Collision_PlayerEyes()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
HRESULT CHuman_Granade::SetUp_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(LEVEL_GAMEPLAY, L"Prototype_Component_Model_Human_Granade", L"Com_Model", (CComponent**)&m_pModelCom, this), E_FAIL);
...
{
	CHuman_Granade*		pInstance = new CHuman_Granade(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CHuman_Granade");
...
{
	CHuman_Granade*		pInstance = new CHuman_Granade(*this);

	if (FAILED(pInstance->Initialize_Clone(wstrPrototypeTag, pArg)))
	{
		MSG_BOX("Failed to Clone : CHuman_Granade");
		Safe_Release(pInstance);
	}
...
void CHuman_Granade::Free()
{
	...
	Safe_Release(m_pHuman_Granade_State);
